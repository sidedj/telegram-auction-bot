...
├── test_persistence.py     # Тестирование системы персистентности
├── test_subscription.py    # Тестирование функции проверки подписки
├── PERSISTENCE_GUIDE.md    # Подробное руководство по персистентности
├── README_PERSISTENCE.md   # Краткое руководство по персистентности
...
Для работы платежей необходимо:

1. **Получить токен провайдера** у платежной системы
2. **Настроить в @BotFather**:
   ```
   /setpayments
   @your_bot_username
   ```
3. **Добавить токен** в переменную `PAYMENTS_PROVIDER_TOKEN`
